<x-layout page-title="Importações">
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="bg-green-100 dark:bg-green-900 border border-green-400 dark:border-green-600 text-green-700 dark:text-green-300 px-4 py-3 rounded-lg">
            {{ session('success') }}
            @if(session('output'))
            <pre class="mt-2 text-xs bg-green-50 dark:bg-green-800 p-2 rounded overflow-x-auto">{{ session('output') }}</pre>
            @endif
        </div>
        @endif
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                            {{ $job->iniciado_em->format('d/m/Y H:i') }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex flex-col gap-1">
                                <span class="text-green-600 dark:text-green-400">
                                    ✓ {{ number_format($job->importados, 0, ',', '.') }} importados
                                </span>
                                @if($job->ignorados > 0)
                                <span class="text-gray-600 dark:text-gray-400">
                                    ⊘ {{ number_format($job->ignorados, 0, ',', '.') }} ignorados
                                </span>
                                @endif
                                @if($job->erros > 0)
                                <span class="text-red-600 dark:text-red-400">
                                    ✗ {{ number_format($job->erros, 0, ',', '.') }} erros
                                </span>
                                @endif
                            </div>
                        </td>
    ...
    <div id="modal-executar" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('import.executar') }}" class="space-y-4">
                    @csrf
